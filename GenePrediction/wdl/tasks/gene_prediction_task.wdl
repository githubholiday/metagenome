...
task GenePredictionTask{
	...
	command <<<
		...
		if [ "$job_state" != "FINISH" ];then
			...
			make -f ~{script}/genePrediction.mk user=~{user} MGM=~{MGM} pro_fa=~{outdir}/~{sample}/~{sample}.pro.fa dna_fa=~{outdir}/~{sample}/~{sample}.dna.fa out_gff=~{outdir}/~{sample}/~{sample}.gff assemble_fa=~{assemble_fa} mod=~{mod} outdir=~{outdir}/~{sample} GenePrediction
