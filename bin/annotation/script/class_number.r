args <- commandArgs(TRUE)
    if (length(args) != 3){
                print("Exmple : inputfile out.stat.xls title")
                q()
        }

cog <- scan(args[1],what="character",sep="
")
...
mtext("Number of Matched genes",side=2,line=2.5,font=2,cex=0.8)
par(cex.axis=0.6)
...
add_len = 13 - length(cog.des)/2
for(i in length(cog.des):1){
text(0.01,i+add_len,cog.des[length(cog.des)+1-i],pos=4,cex=0.7)
}
par(op1)
par(def.par)
