...
class up_down_go_enrich:
    ...
    def Init(self,gene_up_down_enrich_go_file,ax,sample,outdir):
        df = pd.read_csv(gene_up_down_enrich_go_file, header = 0, index_col=False, encoding='utf-8', sep="\t")
        self.df = df
        df['termcolor'] = '#DAD9DE'        
...
def main():
    ...
    ax = fig.add_axes([ax_x, ax_y, ax_w, ax_h], frame_on=True)
    ...
    fig.savefig(os.path.join(args.outDir,"{0}.go.pdf".format(args.sample)),facecolor = fig.get_facecolor(),edgecolor ='none',bbox_inches='tight', dpi=100)
    fig.savefig(os.path.join(args.outDir,"{0}.go.png".format(args.sample)),facecolor = fig.get_facecolor(),edgecolor ='none',bbox_inches='tight', dpi=100)
