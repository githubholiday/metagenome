...
def check_email(table_result,image_result,upload_result,check_flag,project_name,nowtime,indir,qc_type,template,d_type):
	...
	table1 += '<tr><th>质控指标</th><th>参考标准</th><th>审核情况</th><th>备注信息</th><th>异常原因</th><th>建议</th></tr>'
	print(qc_type)
	for qual in qc_type:
		if not "png" in qual and not "upload" in qual:
			if table_result[qual][0]=="不通过":
				print("=========",d_type[qual]["Description"],"======")
				table1 += '<tr><td align=\"center\"><font color=\'red\'> {0} </font></td><td align=\"center\"><font color=\'red\'> {1} </font></td><td align=\"center\"><font color=\'red\'> {2} </font></td><td align=\"center\"><font color=\'red\'> {3} </font></td><td align=\"center\"><font color=\'red\'> {4} </font></td><td align=\"center\"><font color=\'red\'> {5} </font></td></tr>'.format(d_type[qual]["Description"],d_type[qual]["Standard"],table_result[qual][0],table_result[qual][1],d_type[qual]["Exception_Reason"],d_type[qual]["Suggestion"])
			else:
				table1 += '<tr><td align=\"center\">{0}</td><td align=\"center\">{1}</td><td align=\"center\">{2}</td><td align=\"center\">{3}</td><td align=\"center\"> </td><td align=\"center\"> </td></tr>'.format(d_type[qual]["Description"],d_type[qual]["Standard"],table_result[qual][0],table_result[qual][1])
	...
	for sample in samples:
		...
		for qual in quals:
			...
			for rank in d_type[qual]:
				if not rank in ['A','B','C','D']:continue
...
def jump_email(bindir,email_dir,outdir,place):
	# upload the email content and send email
	if place == 'bj':
		print("ssh c0008 /annoroad/share/software/install/Python-3.3.2/bin/python3 {0}/jump_send_email.py -i {1} && cd {2}".format(bindir,email_dir,outdir))
		qc_mail_return = subprocess.call("ssh c0008 /annoroad/share/software/install/Python-3.3.2/bin/python3 {0}/jump_send_email.py -i {1} && cd {2}".format(bindir,email_dir,outdir), shell=True)
		if qc_mail_return == 0:
		...
		print("the place is yw")
		cmd ="python3 {0}/send_email.py {0}/email_config.ini && rm -rf {0}".format(email_dir)
		qc_mail_return = subprocess.call(cmd,shell=True)
		if qc_mail_return == 0:
...
def send_email(result, projectname, outdir, prefix, flag, email_cfg,place):
	...
	if qc_mail:
		...
		if place == "bj":
			...
			email_attachment ="{0}/{1}_QC_Result.zip".format(email_dir,prefix)
		#generate the email_config.ini
		get_email_config(email_cfg, email_subject, email_attachment, email_body, email_dir)
		
		# upload the email content and send email
		jump_email(bindir,email_dir,outdir,place)

def send_check_email(table_result,image_result,upload_result,check_flag,projectname,title,outdir,prefix,indir,qc_type,template,email_cfg,d_type,place):
	...
	if place =="bj":
		...
		zip_attach = "{0}/{1}_QC_Result.zip".format(email_dir,prefix)
	attach_files =[]
		...
		if image_result[qual][0]=="0":
			...
			for f in image_result[qual][1]:
				...
				if place =="bj":
					...
					new_name='{0}/{1}'.format(email_dir,name)
				if num==1:
...
def main():
	...
	parser.add_argument('-pl','--place',help='bj or yw',dest='place',default='bj')
	...
	if args.finish:
		...
		if not args.check:
			...
			send_email(result, projectname, outdir, prefix, args.email, email_cfg,place)
	...
	if not args.check:
		flag=args.email		
		send_email(result, projectname, outdir, prefix, flag, email_cfg,place)
	else:
		send_check_email(table_result,image_result,upload_result,check_flag,projectname,title,outdir,prefix,indir,qc_type,template,email_cfg,d_type,place)
