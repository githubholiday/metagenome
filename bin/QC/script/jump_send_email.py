...
def scp_dir( user , upload_dir ):
	try:
		...
		print(user)
		#print("{0} /home/{1}/{2}/{3} /home/{1}/{2}/{4} && echo ok".format(PYTHON, user, os.path.basename(upload_dir),"send_email.py","email_config.ini"))
		if not myrun(ssh , "scp -r {0}@{1}:{2} /home/{0}  && echo ok ".format(user , IP2, upload_dir)):
			sys.exit("拷贝失败")
		#print("{0} /home/{1}/{2}/{3} /home/{1}/{2}/{4} && echo ok".format(PYTHON, user, os.path.basename(upload_dir))
