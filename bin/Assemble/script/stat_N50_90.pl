...
HEADTEXT
#print $headtext;
my $out_title = sprintf "%-16s%-16s%-16s%-16s%-16s%-16s%-16s%-16s",'#Title','Total_length','Total_number','Num>=100','Num>=2000','Average_length','Max','Min_length';
for (@n_lst) {
...
## Contig
my $res_con=&cal_cov(\@con_s,$totcon_cut,\@n_lst);
...
${$res_con}{'tot_l'}=&separate(${$res_con}{'tot_l'});
${$res_sca}{'tot_l'}=&separate(${$res_sca}{'tot_l'});
${$res_con}{'tot_n'}=&separate(${$res_con}{'tot_n'});
${$res_sca}{'tot_n'}=&separate(${$res_sca}{'tot_n'});
${$res_con}{'max'}=&separate(${$res_con}{'max'});
${$res_sca}{'max'}=&separate(${$res_sca}{'max'});
$con2k=&separate($con2k);
$sca2k=&separate($sca2k);

print "Items\tContig_len(bp)\tContig_num\n";
print "Total\t${$res_con}{'tot_l'}\t${$res_con}{'tot_n'}\n";
print "Max\t${$res_con}{'max'}\t-\n";
print "Number>=2000bp\t-\t$con2k\n";
for (@n_lst) {
    ${$res_con}{'l'.$_}=&separate(${$res_con}{'l'.$_});
    ${$res_sca}{'l'.$_}=&separate(${$res_sca}{'l'.$_});
    ${$res_con}{'n'.$_}=&separate(${$res_con}{'n'.$_});
    ${$res_sca}{'n'.$_}=&separate(${$res_sca}{'n'.$_});
    print "N${_}0\t${$res_con}{'l'.$_}\t${$res_con}{'n'.$_}\n";
}
