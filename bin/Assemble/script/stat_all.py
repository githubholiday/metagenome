...
def main():
    ...
    #title = 'Sample\tContig\tContig bases(bp)\tN50(bp)\tN60(bp)\tN70(bp)\tN80(bp)\tN90(bp)\tMax(bp)\tNumber>=2000bp\n'
    title = 'Sample\tContig\tContig bases(bp)\tN50(bp)\tN90(bp)\tMax(bp)\tNumber>=2000bp\n'
    args.output.write(title)
    for file in args.input :
        check_file_exists( file )
        sample = os.path.basename(file).split('.')[0]
        dict = read_file( file )
        #out = [ sample, dict['Total'][1], dict['Total'][0], dict['N50'][0], dict['N60'][0], dict['N70'][0], dict['N80'][0], dict['N90'][0], dict['Max'][0], dict['Number>=2000bp'][1]]
        out = [ sample, dict['Total'][1], dict['Total'][0], dict['N50'][0], dict['N90'][0], dict['Max'][0], dict['Number>=2000bp'][1]]
