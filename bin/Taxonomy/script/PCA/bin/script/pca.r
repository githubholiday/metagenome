...
######++++++++++++++++++Table Output End++++++++++++++++++++
	...
	coltmp <- c("red","darkgreen","blue","black","chocolate","brown","darkorange",
			"deeppink","deepskyblue1","lawngreen","yellow","grey","tomato","purple","grey","gold",
			"cyan","darkmagenta","darkred")
...
#	col<-sampleCol$V2
	...
	scatterplot3d(res_pca$ind$coord[,1:3],color=colors,pch=16,xlab=dim1,ylab=dim2,zlab=dim3,cex.symbols=1.5,cex.lab=1.5,cex.axis=1.2)
#	print(group)
#	print(unique(group))
#	legend("topright",pch=16,legend=as.character(unique(group)),bty = 'n',col = col[1:length(unique(comm_input$group))])
	legend("right",pch=16,legend = levels(cmp$group),bty = 'n',col =  coltmp[1:len_group], cex=0.6,inset = 0.1,ncol=2)
	dev.off()
############################++++++++++++++++3d plot End+++++++++++++++++++++++++++++++++++
...
main_fun <- function()
{
	...
	cat(mess,"\n")
	pca_analysis(fpkm,parent_out_dir,cmp,"FALSE")
}
