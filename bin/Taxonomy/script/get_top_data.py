...
def main():
    ...
    parser.add_argument('-n','--number',help='top number',dest='number',type=int,required=True)
    ...
    out_df = df[0:args.number]
    out_df.to_csv(args.output,sep='\t',index=True,header=True)
    ...
    for c in out_df.columns :
        ...
        # tmp = 1-int(sum)
        # print ( sum, tmp )
        out.append( str(100-out_df[c].sum()) )
