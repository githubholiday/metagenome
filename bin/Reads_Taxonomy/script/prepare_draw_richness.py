...
def main():
	...
	top_plot_list = args.outdir+"top_"+str(args.top)+"list"
	list_file = open(top_plot_list, 'w' )
	with open(all_sample_stat,'w') as out:
		...
		for i in range(len(tax_dic)):
			...
			top_file = "{0}/{1}_{2}_{3}_top.xls".format(args.outdir+,str(i),tax_name[i],str(args.top))
			top_pdf = "{0}/{1}_{2}_{3}_top".format(args.outdir+,str(i),tax_name[i],str(args.top))
			list_file.write( top_file+'\t'+top_pdf+'\n')
