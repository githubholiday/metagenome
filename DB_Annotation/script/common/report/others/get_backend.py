...
def main():
	...
	if os.path.isfile(args.config):
		...
		if template:
			for i in template:
				if os.path.isfile(template[i]):
					os.system("mkdir -p {0}/upload/tools".format(shell["~{reportdir}"]))
					os.system("cp -rf {0} {1}/upload/tools".format(template[i],shell["~{reportdir}"]))
				else:
					os.system("sed -i 's#{0}#{1}#g' {2}/template.md ".format(i,template[i].strip("\n"),shell["~{reportdir}"]))
		if template_new:
			for j in template_new:
				if os.path.isfile(template_new[j]):
					os.system("mkdir -p {0}/upload/tools".format(shell["~{reportdir}"]))
					os.system("cp -rf {0} {1}/upload/tools".format(template_new[j],shell["~{reportdir}"]))
				else:
					print(template_new[j])
					os.system("sed -i 's#$({0})#{1}#g' {2}/template.md ".format(j,template_new[j].strip("\n"),shell["~{reportdir}"]))
	#replace backend.sh
